...
class Boid(PhysicsObjet):
    ...
    def separation(self):
        ...
        for boid in self.simulation.boids:
            if boid != self:
                distance = self.pos - boid.pos
                if 0 < distance.length() < SEPARATION_RADIUS:
                    force_vector += distance.normalize() / distance.length()
    ...
    def alignment(self):
        ...
        :return force vector
        """
        average_heading = pg.math.Vector2(0, 0)
        count = 0
        for boid in self.simulation.boids:
            if boid != self:
                distance = self.pos - boid.pos
                if 0 < distance.length() < ALIGNMENT_RADIUS:
                    average_heading += boid.vel
                    count += 1
        if count > 0:
            average_heading /= count
            if average_heading.length_squared() > 0:
                average_heading.normalize_ip()
            force_vector = average_heading - self.vel
            limit(force_vector, self.max_force)
            return force_vector
        else:
    ...
    def cohesion(self):
        ...
        """
        average_position = pg.math.Vector2(0, 0)
        count = 0
        for boid in self.simulation.boids:
            if boid != self:
                distance = self.pos - boid.pos
        ...
        if count > 0:
            ...
            force_vector = self.move_towards_target(average_position)
            return force_vector
        else:
            return pg.math.Vector2(0, 0)
